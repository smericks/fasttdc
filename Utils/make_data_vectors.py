...
def fpd_gamma_samples(x_im,y_im,npe_mu,npe_cov,num_fpd_samps):
    """
    Args:
        x_im (num_lenses,num_images)
        y_im (num_lenses,num_images)
        npe_mu (num_lenses,num_lens_params)
        npe_cov (num_lenses,num_lens_params,num_lens_params)
        num_fpd_samples

    Returns:
        fpd_samps (num_lenses,num_fpd_samps,num_images), 
        lens_param_samps (num_lenses,num_fpd_samps,num_lens_params)
    """
    size_subsamp = len(npe_mu)
    num_images = np.shape(x_im)[-1]
    fpd_samps = np.zeros((size_subsamp,num_fpd_samps,num_images-1)) # dbls padded w/ zeros
    lp_samps = np.empty((size_subsamp,num_fpd_samps,np.shape(npe_mu)[-1]))

    if num_images==2:
        for i in range(0,size_subsamp):
            lens_param_samps = multivariate_normal.rvs(mean=npe_mu[i],
                cov=npe_cov[i],size=num_fpd_samps)
            fermatpot_samps = batched_fermatpot.eplshear_fp_samples(x_im[i],
                y_im[i],lens_param_samps[:,:8],lens_param_samps[:,8],
                lens_param_samps[:,9])
