...
class LensCatalog:
    ...
    def populate_image_positions(self):
        ...
        im1 = self.lens_df.loc[:,'x_im1'].to_numpy().astype(float)
        self.dbls_idxs = np.where(np.isnan(im3) & np.isnan(im2) & ~np.isnan(im1))[0]
    ...
    def populate_truth_sigma_v_4MOST(self):
        ...
        def ground_truth_veldisp(theta_E,gamma_lens,R_sersic,n_sersic,
            ...
            corrected_distance_ratio = np.sqrt(
                tdc_utils.kin_distance_ratio(self.my_cosmology,z_lens,z_src))

            vel_disp_numerical *= (corrected_distance_ratio/default_distance_ratio)

            return vel_disp_numerical
    
        truth_sigma_v = np.empty((len(self.lens_df)))
        for r in range(0,len(self.lens_df)):
            sigma_v = ground_truth_veldisp(
                self.lens_df.loc[r,'theta_E'],
                self.lens_df.loc[r,'gamma'],
                self.lens_df.loc[r,'lens_light_parameters_R_sersic'],
                self.lens_df.loc[r,'lens_light_parameters_n_sersic'],
                self.lens_df.loc[r,'z_lens'],
                self.lens_df.loc[r,'z_src'])
            truth_sigma_v[r] = sigma_v
            
        self.lens_df['sigma_v_4MOST_truth'] = truth_sigma_v
