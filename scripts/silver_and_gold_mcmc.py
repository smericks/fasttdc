...
silver_prefactor = np.append(gold_prefactor,inputs_dict['silver_quads']['prefactor'])
silver_prefactor = np.append(silver_prefactor,inputs_dict['silver_dbls']['prefactor'])
...
silver_z_lens = np.append(gold_z_lens,inputs_dict['silver_quads']['z_lens_truth'])
silver_z_lens = np.append(silver_z_lens,inputs_dict['silver_dbls']['z_lens_truth'])

gold_z_src = np.append(
    inputs_dict['gold_quads']['z_src_truth'],
    inputs_dict['gold_dbls']['z_src_truth']
)
silver_z_src = np.append(gold_z_src,inputs_dict['silver_quads']['z_src_truth'])
silver_z_src = np.append(silver_z_src,inputs_dict['silver_dbls']['z_src_truth'])
...
h5f = h5py.File((exp_folder+'/combined_chain_5e3_w0waCDM.h5'), 'w')
