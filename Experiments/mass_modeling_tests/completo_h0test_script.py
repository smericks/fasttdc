...
CHAIN_PATH = 'InferenceRuns/mdn/with_lenscenter_200lenses_backend.h5'
...
dv_dict_dbls_mdn, dv_dict_quads_mdn = dv_dict_from_completo(
...
lklhd_obj_dbls = tdc_sampler.TDCLikelihood(
    fpd_sample_shape=np.shape(dv_dict_dbls_mdn['fpd_samples']), # pre-define input shape for fermat potential differences
...
lklhd_obj_quads = tdc_sampler.TDCLikelihood(
    fpd_sample_shape=np.shape(dv_dict_quads_mdn['fpd_samples']), # pre-define input shape for fermat potential differences
    cosmo_model='LCDM', # infers four params: [H0,OmegaM,mu(gamma),sigma(gamma)]
    use_astropy=True, # relic option, we always use astropy right now
    use_gamma_info=True) # whether to infer a population over gamma_lens or not
...
my_chain_nsf = tdc_sampler.fast_TDC([lklhd_obj_quads,lklhd_obj_dbls], 
    [dv_dict_quads_mdn,dv_dict_dbls_mdn], num_emcee_samps=500, # DEBUG: make sure everything works!
    n_walkers=20, use_mpi=False, use_multiprocess=True, backend_path=CHAIN_PATH, # <-- TODO save path
