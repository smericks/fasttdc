...
class PaltasPreds:
    ...
    def create_image(self,lens_df_row):
        ...
        # POINT SOURCE
        for key in ['mag_app','x_point_source','y_point_source']:
    ...
    def preds_from_params(self,lens_df):
        """Takes in a pd.Series object that contains 
        Args:
            lens_df_row (pd.Series)
        """
        images = []
        num_ims = len(lens_df)
        for r in range(0,len(lens_df)):
            im = self.create_image(lens_df.loc[r])
        ...
        images = tf.reshape(images,[num_ims,self.img_size[0],self.img_size[1],self.img_size[2]])

        y_pred,std_pred,cov_pred = self._process_image_batch(images)
